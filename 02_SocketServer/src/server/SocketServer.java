	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			while(true){
				...
				int iNumero3 = Integer.parseInt(operadores[2]);// Codigo de operación (Suma/Resta/Producto/Division)
				
				// Calcular según el código de operacion
				if (iNumero3==1) {
					int resultado = iNumero1 + iNumero2;// 7 (suma 3+4)
					//Thread.sleep(10000);//simulamos que tardamos un tiempo en calcular
					salida = new PrintStream(socketConexion.getOutputStream());
					salida.println(resultado);
				} else if (iNumero3==2) {
					int resultado = iNumero1 - iNumero2;// -1 (resta 3-4)
					salida = new PrintStream(socketConexion.getOutputStream());
					salida.println(resultado);
				} else if (iNumero3==3) {
					int resultado = iNumero1 * iNumero2;// 12 (producto 3*4)
					salida = new PrintStream(socketConexion.getOutputStream());
					salida.println(resultado);
				} else if (iNumero3==4) {
					int resultado = iNumero1 / iNumero2;// 0.75 (división 3/4)
					salida = new PrintStream(socketConexion.getOutputStream());
					salida.println(resultado);
				}
